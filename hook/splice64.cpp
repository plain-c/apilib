...
namespace apilib
{
  ...
  */
  ...
  /**
   * Контекст сплайсинга в 64-битном окружении.
  */
  struct splice_t 
  {
  ...
  */
  static void* splice_get_jump_address(void* address)
  {
    ...
    do 
    {
      ...
      if(buffer[0] == 0xEB)
      {
        r = (void*)((unsigned char*)address + 2 + (int64_t)(*(int08_t*)&buffer[1])); 
        break;
      }
      ...
      if(buffer[0] == 0xE9)
      {
        r = (void*)((unsigned char*)address + 5 + (int64_t)(*(int32_t*)&buffer[1]));
        break;
      }

      // Проверка инструкции 'jmp' (absolute 64-bit) без REX.W префикса.
      if(buffer[0] == 0xFF && buffer[1] == 0x25) 
      {
        void* tmp = NULL;
        void* ptr = (void*)((unsigned char*)address + 6 + (int64_t)(*(int32_t*)&buffer[2]));
      ...
      // Проверка инструкции 'jmp' (absolute 64-bit) с REX.W префиксом.
      if(buffer[0] == 0x48 && buffer[1] == 0xFF && buffer[2] == 0x25)
      {
        void* tmp = NULL;
        void* ptr = (void*)((unsigned char*)address + 7 + (int64_t)(*(int32_t*)&buffer[3]));
        if(spliceimport_memread(ptr, &tmp, sizeof(tmp)) == true)
        {
          r = tmp;
        }
